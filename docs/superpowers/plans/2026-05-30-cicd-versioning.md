...
- `apps/desktop/electron-builder.json` — the publish target (`github`, `ecogs-sys/AI.Pad`) is already set; `--publish always` consumes it.
...
Write `C:\Work\ecogs\projects\AI.Pad\release-please-config.json` with this exact content:
...
Write `C:\Work\ecogs\projects\AI.Pad\.release-please-manifest.json` with this exact content:
...
Write `C:\Work\ecogs\projects\AI.Pad\.github\workflows\release-please.yml` with this exact content:
...
@aipad/desktop dist' and should never be committed. CI uploads
...
Fill in:
- **Token name:** `AI.Pad release-please bot`
...
- **Repository access:** _Only select repositories_ → choose `ecogs-sys/AI.Pad`.
...
Open: https://github.com/ecogs-sys/AI.Pad/settings/secrets/actions
...
Reload https://github.com/ecogs-sys/AI.Pad/settings/secrets/actions.
...
Open: https://github.com/ecogs-sys/AI.Pad/actions/workflows/release-please.yml
...
Open: https://github.com/ecogs-sys/AI.Pad/pulls?q=is%3Apr+is%3Aopen+author%3Aapp%2Fgithub-actions
...
Open: https://github.com/ecogs-sys/AI.Pad/actions/workflows/release.yml
...
Open: https://github.com/ecogs-sys/AI.Pad/releases/tag/v0.4.0

Expected six assets attached:
- `AI.Pad-Setup-0.4.0.exe` (Windows installer)
- `AI.Pad-0.4.0.dmg` (macOS installer)
- `AI.Pad-0.4.0.AppImage` (Linux installer)
- `latest.yml`           (electron-updater feed for Windows)
- `latest-mac.yml`       (feed for macOS)
- `latest-linux.yml`     (feed for Linux)

(Exact filenames depend on electron-builder's defaults + the `productName` in `electron-builder.json` which is `AI.Pad`; the count is the canonical check.)
...
```
First release-please-driven release: v0.4.0 — see https://github.com/ecogs-sys/AI.Pad/releases/tag/v0.4.0
...
- ✅ **Type consistency:** secret name `RELEASE_PLEASE_TOKEN` is identical in Task 2 (workflow reference) and Task 5 (UI step). Manifest paths `.` and `apps/desktop` are identical in Task 1.1 (config), Task 1.3 (manifest), and Task 7.4 (diff check). Package names `aipad` and `@aipad/desktop` are identical in config + linked-versions components.
